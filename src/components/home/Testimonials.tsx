...
const Testimonials = () => {
  ...
  const getPrevIndex = (): number => (currentIndex - 1 + testimonials.length) % testimonials.length;
  ...
  return (
    <div className="py-12 md:py-20 overflow-hidden">
      <div className="w-full flex items-center justify-center flex-col gap-3 px-4">
        <h1 className="font-normal text-2xl sm:text-3xl md:text-4xl lg:text-5xl leading-tight tracking-tight text-center">
          Stories of Impact and Growth
        </h1>
        <p className="max-w-[725px] font-normal text-sm sm:text-base md:text-lg leading-relaxed text-center">
          Real people. Real change. Hear from those who are investing in Nigeria's future through SACUDI
        </p>
      </div>
      ...
      <div className="relative px-4 sm:px-6 md:px-10 py-10 md:py-14 max-w-6xl mx-auto">
        ...
        <div className="hidden md:flex items-center justify-center gap-4 mb-8">
          <PreviewCard testimonial={testimonials[getPrevIndex()]} onClick={() => goToSlide(getPrevIndex())} />
          <MainCard testimonial={testimonials[currentIndex]} />
          <PreviewCard testimonial={testimonials[getNextIndex()]} onClick={() => goToSlide(getNextIndex())} />
        </div>

        {/* Mobile Layout - Swipeable */}
        <div className="md:hidden space-y-6" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          <MainCard testimonial={testimonials[currentIndex]} />
          <div className="flex justify-between gap-4">
            <MobilePreview testimonial={testimonials[getPrevIndex()]} onClick={() => goToSlide(getPrevIndex())} />
            <MobilePreview testimonial={testimonials[getNextIndex()]} onClick={() => goToSlide(getNextIndex())} />
          </div>
        </div>

        <div className="flex justify-center mt-8 md:mt-10">
          <Button
            text="Read More Impact Stories"
            textColor="#007F8E"
            borderColor="#007F8E"
        ...
        <div className="flex justify-center mt-6 space-x-2">
          {testimonials.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${currentIndex === index ? 'bg-[#007F8E] w-6' : 'bg-gray-300'}`}
...
// Component for main testimonial card
const MainCard = ({ testimonial }: { testimonial: Testimonial }) => (
  <motion.div
    className="w-full bg-[#C4E2EA] py-5 px-5 rounded-lg"
...
// Component for desktop preview cards
const PreviewCard = ({ testimonial, onClick }: { testimonial: Testimonial, onClick: () => void }) => (
  <motion.div 
    onClick={onClick}
...
// Component for mobile preview cards
const MobilePreview = ({ testimonial, onClick }: { testimonial: Testimonial, onClick: () => void }) => (
  <motion.div 
    onClick={onClick}
